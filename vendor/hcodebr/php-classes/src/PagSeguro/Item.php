...
class Item
{
    ...
    {
        if (!$id || !$id > 0) throw new Exception("Informe o ID do item.");
        if (!$description) throw new Exception("Informe a descrição do item.");
        if (!$amount || !$amount > 0) throw new Exception("Informe o valor total do item.");
        if (!$quantity || !$quantity > 0) throw new Exception("Informe a quantidade do item.");
